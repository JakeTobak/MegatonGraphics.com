...
break; } $cart_products = $this->cart->getProducts(); 
				foreach ($this->request->post['quantity'] as $key => $value) {
...
$this->data['copu_order'] = $this->getChild('myoc/copu', array('type' => 'order', 'path' => 'cart'));
...
if($product_option['type'] == 'file' && $this->config->get('upload_product_status') && $this->config->get('upload_product_store') != "" && in_array($this->config->get('config_store_id'), explode(',', $this->config->get('upload_product_store'))) && in_array($product_option['option_id'], explode(',', $this->config->get('upload_product_option')))) {
