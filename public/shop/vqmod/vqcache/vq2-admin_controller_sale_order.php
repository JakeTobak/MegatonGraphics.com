...
$this->data['tab_copu'] = $this->language->get('tab_copu');
				...
				$this->data['copu_product_js'] = $this->getChild('myoc/copu/product', array('type' => 'product', 'js' => true));
		$this->children = array(
			'common/header',
			'common/footer'
		);
				...
				foreach ($options as $option) {
					if ($option['type'] != 'file') {
						...
						$option_data[] = array(
							...
							'type'  => $option['type'],
'value' => (function_exists('utf8_substr') ? utf8_substr($option['value'], 0, utf8_strrpos($option['value'], '.')) : substr($option['value'], 0, strrpos($option['value'], '.'))) . '</a> [' .formatFilesize(filesize(DIR_DOWNLOAD . $option['value'])) . ']<a>',
...
$this->data['tab_copu'] = $this->language->get('tab_copu');
				$this->data['copu_order'] = $this->getChild('myoc/copu', array('type' => 'order', 'edit' => false));
			$this->children = array(
				'common/header',
				'common/footer'
			);
...
$this->data['copu_order'][$order_id] = $this->getChild('myoc/copu/invoice', array('order_id' => $order_id));
			$order_info = $this->model_sale_order->getOrder($order_id);
